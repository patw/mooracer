...
- **Request**: `version` + `collection` + a **command union** (one payload per
  ...
  `VectorSearch`/`TextSearch`/`HybridSearch`/`Group`/`Stats`). `one`/`many`
  flags collapse `*_one`/`*_many` onto one payload each; `limit == 0` = no limit;
  `filter {}` = all.
- **Response**: `version` + `status` + `message` + a **response union** body
  ...
  `GroupRes{groups}`, `StatsRes{…}`). `body` is present only on `OK`.
...
create the collection on first use. **Wire v1 has no index-management command**
(indexes are server-side only).
...
`wire/` subpackage generated by `flatc --python` from the same schema.
...
Because wire v1 has no index-management command, the Python **tests** drive a
dev server (`server/src/bin/mooracer-devserver.rs`) that pre-creates
vector/text indexes from env (`MOORACER_VECTOR_INDEX=coll:field:dim;…`,
`MOORACER_TEXT_INDEX=coll:field;…`) before serving. Run the Python suite with:
